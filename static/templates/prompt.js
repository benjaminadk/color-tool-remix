...
docReady(() => {
  ...
  document.getElementById('ok').textContent = promptOptions.ok

  if (promptOptions.type === 'alert') {
    document.getElementById('cancel').remove()
  } else {
    document.getElementById('cancel').addEventListener('click', () => promptCancel())
    document.getElementById('cancel').textContent = promptOptions.cancel
  }
  ...
  if (promptOptions.type === 'input') {
    ...
    dataEl.textContent = promptOptions.message
    dataEl.style.cssText = 'font-size:11px;text-align:justify;'
  }

  dataContainerEl.appendChild(dataEl)
  dataEl.setAttribute('id', 'data')

  if (['input', 'select'].includes(promptOptions.type)) {
    dataEl.focus()
    if (promptOptions.type === 'input') {
      dataEl.select()
    }
